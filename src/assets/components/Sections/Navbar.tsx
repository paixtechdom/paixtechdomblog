        ...
        <>
           <header className='w-full center  bg-gradient-to-l from-secondary via-primary to-secondary fixed min-h-[8vh] lg:min-h-[13vh] top-0 left-0 z-50 border-b border-blue-900'>
                <div className="w-11/12 lg:w-10/12 xl:w-9/12 flex justify-between items-center">
                    ...
                    <nav className={`${showNav ? 'left-0' : '-left-[25vw] opacity-0 lg:opacity-100 lg:left-0'} transition-all duration-1000 absolute py-9 lg:py-0 bg-gradient-to-l from-secondary via-primary to-secondary lg:from-transparent w-full lg:w-fit top-[8vh]  lg:top-0 lg:relative flex flex-col lg:flex-row gap-9 lg:gap-7 xl:gap-9 items-center border-b border-blue-900 lg:border-b-0`}>
                        {
                            Navs.map((nav, i) => (
                                <Link key={i} to={`/${nav.link}`} className={`flex w-11/12 lg:w-fit gap-2 items-center ${currentNav == i ? 'border-b-2 border-blue-900 font-bold text-blue-600' : 'text-white'} hover:text-blue-600 hover:border-b hover:border-blue-400`} onClick={() => {
                                    setCurrentNav(i)
                                    setShowNav(false)
                                    }}>
